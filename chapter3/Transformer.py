...
class MultiHeadAttention(nn.Module):
    ...
    def scaled_dot_product_attention(self, Q, K, V, mask=None):
        # 1. 计算注意力分数 (QK^T, K^T 是 K 的转置)
        attn_scores = torch.matmul(Q, K.transpose(-2, -1)) / math.sqrt(self.d_k)

        # 2. 应用掩码（如果有）
        if mask is not None:
            # 将掩码中为 0 的位置设置为一个非常大的负数，这样 softmax 后会接近 0
            attn_scores = attn_scores.masked_fill(mask == 0, -1e9)
        
        # 3. 计算注意力权重（Softmax），dim = -1 表示在最后一个维度上进行 softmax
    ...
    def combine_heads(self, x):
        ...
        # 变回 (batch_size, seq_alength, d_model)
...
class Encoder(nn.Module):
    def __init__(self, vocab_size, d_model, num_layers, num_heads, d_ff, dropout, max_len):
        ...
        self.pos_encoder = PositionEncoding(d_model, dropout, max_len)
...
if __name__ == "__main__":
    ...
    # 3. 创建模拟输入数据
    # 假设 batch_size = 2, src_len = 10, tgt_len = 12
    ...
    # 预期输出: torch.size([2, 12, 5000])，即 (batch_size, tgt_seq_len, tgt_vocab_size)
