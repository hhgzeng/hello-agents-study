...
知识图谱,技术组件,0.9,实体关系的结构化表示"""
    ...
    def demonstrate_markitdown_conversation(self, file_paths):
        """演示MarkItDown转换过程"""
        print("\n🔄 MarkItDown转换过程演示")
        print("-" * 50)
        ...
        for filename, file_path in file_paths.items():
            ...
            result = self.rag_tool.run(
                {"action": "add_document", "file_path": file_path}
            )
...
"""
    ...
    def demonstrate_pipeline_performance(self):
        ...
        batch_result = self.rag_tool.batch_add_texts(
            batch_texts, document_ids=[f"perf_test_{i+1}" for i in range(10)]
        )
...
def main():
    ...
    try:
        ...
        conversion_results = demo.demonstrate_markitdown_conversation(file_paths)
