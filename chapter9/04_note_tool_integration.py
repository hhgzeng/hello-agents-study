...
from hello_agents.tools import NoteTool  # MemoryTool, RAGTool
...
class ProjectAssistant(SimpleAgent):
    ...
    def _save_as_note(self, user_intput: str, response: str):
        try:
            if "问题" in user_intput or "阻塞" in user_intput:
                note_type = "blocker"
            elif "计划" in user_intput or "下一步" in user_intput:
                note_type = "action"
            else:
